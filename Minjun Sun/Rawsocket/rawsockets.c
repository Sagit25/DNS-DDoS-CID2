/* copied from https://github.com/MaxXor/raw-sockets-example/ */
/* how to use it >> ./rawsocket <source ip> <dest ip> <port> */
...
void create_ack_packet(struct sockaddr_in* src, struct sockaddr_in* dst, int32_t seq, int32_t ack_seq, char** out_packet, int* out_packet_len)
{
	...
	struct iphdr* iph = (struct iphdr*)datagram;
	struct tcphdr* tcph = (struct tcphdr*)(datagram + sizeof(struct iphdr));
	struct tcp_opt_2* opt_pflag = (struct tcp_opt_2*)(datagram + sizeof(struct iphdr) + sizeof(struct tcphdr));
	struct tcp_opt_6* opt_puzzle = (struct tcp_opt_6*)(datagram + sizeof(struct iphdr) + sizeof(struct tcphdr) + sizeof(struct tcp_opt_2));
	struct pseudo_header psh;
...
int main(int argc, char** argv)
{
	...
	int iter = 0, iter_limit = 5;

	// receive response
	while (iter < iter_limit && (received = receive_from(sock, recvbuf, sizeof(recvbuf), &saddr)) > 0)
	{
